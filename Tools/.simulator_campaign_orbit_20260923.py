...
def json_safe(value: Any) -> Any:
    """Replace non-finite telemetry numbers so one sample cannot end recording."""
...
def atomic_json(path: Path, value: dict[str, Any]) -> None:
    ...
    temp.write_text(json.dumps(json_safe(value), separators=(",", ":"), allow_nan=False), encoding="utf-8")
...
class SnapshotRecorder:
    ...
    def __call__(self, snapshot: dict[str, Any]) -> None:
        ...
        self.handle.write(json.dumps(json_safe(wrapped), separators=(",", ":"), allow_nan=False) + "\n")
        self.count += 1
        ...
        mirror["simulation"] = {
            ...
            "rateMode": self.manifest["rateMode"],
            "lockstep": True,
...
def main() -> int:
    ...
    ap.add_argument("--planned-orbit-burn", action="store_true",
                    help="Let full Guidance choose and execute the deorbit burn from the supplied preburn orbit.")
    ...
    ap.add_argument("--planning-timeout", type=float, default=180.0,
                    help="Wall time to wait for the full deorbit planner; does not change simulation pacing or Guidance policy.")
    ap.add_argument("--sim-rate", default="max",
                    help="ShuttleSim wall-clock pacing: max or simulated seconds per real second.")
    ap.add_argument("--command-port", type=int, default=19895)
    ap.add_argument("--telemetry-port", type=int, default=19896)
    ap.add_argument("--web-telemetry-port", type=int, default=0)
    ...
    orbital_burn_model: dict[str, Any] | None = None
    if args.planned_orbit_burn:
        if args.engage != "full":
            raise ValueError("--planned-orbit-burn requires --engage full")
        if args.preroll_seconds != 0.0:
            raise ValueError("--planned-orbit-burn requires --preroll-seconds 0 to preserve the preburn orbit state")
        values: dict[str, str] = {}
        for raw in args.scenario.read_text(encoding="utf-8").splitlines():
            item = raw.split("#", 1)[0].strip()
            if "=" in item:
                key, value = item.split("=", 1)
                values[key.strip()] = value.strip()
        mass = float(values["mass_kg"])
        reference_delta_v = float(values["deorbit_delta_v_mps"])
        reference_duration = float(values["deorbit_duration_s"])
        if not (mass > 0.0 and reference_delta_v > 0.0 and reference_duration > 0.0):
            raise ValueError("The source scenario must include a measured deorbit delta-v, duration, and mass")
        reference_throttle = float(guidance.get("deorbitMaximumThrottle", 1.0))
        if not (0.0 < reference_throttle <= 1.0):
            raise ValueError("The configured deorbit throttle fraction must be between zero and one")
        available_thrust = mass * reference_delta_v / (reference_duration * reference_throttle)
        guided_lines = []
        replaced = {"name", "deorbit_delta_v_mps", "deorbit_duration_s", "deorbit_delay_s",
                    "orbital_engine_available_thrust_n"}
        seen: set[str] = set()
        for raw in args.scenario.read_text(encoding="utf-8").splitlines():
            item = raw.split("#", 1)[0].strip()
            key = item.split("=", 1)[0].strip() if "=" in item else ""
            if key == "name":
                guided_lines.append(f"name={values.get('name', 'ShuttleSim')}-guided-orbit")
                seen.add(key)
            elif key == "deorbit_delta_v_mps":
                guided_lines.append("deorbit_delta_v_mps=0")
                seen.add(key)
            elif key in ("deorbit_duration_s", "deorbit_delay_s"):
                guided_lines.append(f"{key}=0")
                seen.add(key)
            elif key == "orbital_engine_available_thrust_n":
                guided_lines.append(f"orbital_engine_available_thrust_n={available_thrust:.9f}")
                seen.add(key)
            else:
                guided_lines.append(raw)
        for key in replaced - seen:
            if key == "name":
                guided_lines.append(f"name={values.get('name', 'ShuttleSim')}-guided-orbit")
            elif key == "deorbit_delta_v_mps":
                guided_lines.append("deorbit_delta_v_mps=0")
                ...
                guided_lines.append(f"orbital_engine_available_thrust_n={available_thrust:.9f}")
        effective_scenario = run_dir / "scenario-guided-orbit.ini"
        effective_scenario.write_text("\n".join(guided_lines) + "\n", encoding="utf-8")
        orbital_burn_model = {
            "mode": "guidance-planned",
            "referenceScenario": str(args.scenario),
            "referenceDeltaV_mps": reference_delta_v,
            "referenceDuration_s": reference_duration,
            "referenceMass_kg": mass,
            "referenceThrottleFraction": reference_throttle,
            "derivedAvailableThrust_N": available_thrust,
            "preprogrammedBurn": False,
        }
    ...
    manifest: dict[str, Any] = {
        ...
        "sourceScenario": str(args.scenario),
        "config": str(args.config),
        "startedAt": time.time(),
        "rateMode": args.sim_rate,
        "lockstep": True,
        "physicsDt": 0.02,
        "guidanceRateHz": guidance_rate,
        "prerollSeconds": args.preroll_seconds,
        "orbitalBurnModel": orbital_burn_model,
    ...
    sim_cmd = [
        ...
        "--web-telemetry-port", str(args.web_telemetry_port),
    ...
    try:
        ...
        os.environ["KSP_LANDER_SIM_COMMAND_PORT"] = str(args.command_port)
        os.environ["KSP_LANDER_SIM_TELEMETRY_PORT"] = str(args.telemetry_port)
        os.environ["KSP_LANDER_SIM_WEB_TELEMETRY_PORT"] = str(args.web_telemetry_port)
        ...
        preroll_socket.bind(("127.0.0.1", args.telemetry_port))
        ...
        try:
            ...
            while True:
                ...
                if sim_time + 1e-9 >= args.preroll_seconds:
                    break
                preroll_cmd.sendto(b'{"type":"step","step":true}',
                    ("127.0.0.1", args.command_port))
        ...
        os.environ["KSP_LANDER_ENTRY_WORKER_TRACE"] = "1"
        os.environ["KSP_LANDER_TRACE_TOPOLOGY_TIMING"] = "1"
        os.environ["KSP_LANDER_TRACE_TOPOLOGY_DETAIL"] = "1"
        os.environ["KSP_LANDER_TRACE_TOPOLOGY_GRID"] = "1"
        ...
        if args.engage == "full":
            os.environ.pop("KSP_LANDER_FORCE_REENTRY_TEST", None)
        else:
        ...
        if args.engage not in ("hac", "hac-upstream"):
            with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as wake:
                wake.sendto(b'{"type":"step","step":true}',
                    ("127.0.0.1", args.command_port))
        ...
        if args.engage == "full":
            backend.send("createPlan", timeout=args.planning_timeout)
        ...
        if not armed.get("automationEngaged"):
            try:
                armed = backend.read_until(
                    lambda s: bool(s.get("automationEngaged")) or
                    str(s.get("phase") or "") in TERMINAL_PHASES,
                    10.0,
                )
            except TimeoutError:
                armed = backend.latest_snapshot or {}
