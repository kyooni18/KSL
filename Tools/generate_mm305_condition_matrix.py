...
def compile_case(base: dict[str, str], out: Path, heading: float | None, altitude: float,
    ...
    # "auto" points the air-relative velocity along the great-circle bearing
    # from the actual displaced vehicle position toward KSC.  This naturally gives
    # south -> 0 deg, north -> 180 deg, east -> 270 deg, and west -> 90 deg without
    # encoding any cardinal-position special cases.
    resolved_heading = heading
    if resolved_heading is None:
        vehicle_lat, vehicle_lon = lla_from_position(p, ut0)
        dlon = runway_lon - vehicle_lon
        y = math.sin(dlon) * math.cos(runway_lat)
        x = (math.cos(vehicle_lat) * math.sin(runway_lat)
             - math.sin(vehicle_lat) * math.cos(runway_lat) * math.cos(dlon))
        if abs(x) <= math.ulp(1.0) and abs(y) <= math.ulp(1.0):
            resolved_heading = 0.0
        else:
            resolved_heading = math.degrees(math.atan2(y, x)) % 360.0
    v = inertial_velocity(p, ut0, resolved_heading, speed, fpa)
    heading_slug = "auto" if heading is None else f"{resolved_heading:g}"
    slug = f"h{heading_slug}-a{altitude:g}-v{speed:g}"
    if east_offset_m or north_offset_m:
    ...
    lines = [
        ...
        f"# Condition: air-relative heading={resolved_heading:g} deg "
        f"({'auto KSC-bearing' if heading is None else 'explicit'}) "
        f"altitude={altitude:g} m "
    ...
    out.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return {
        "name": name, "scenario": str(out), "heading_deg": resolved_heading,
        "heading_mode": "inbound-perpendicular" if heading is None else "explicit",
...
def main() -> int:
    ...
    parser.add_argument("--headings", default="auto",
                        help="comma-separated air-relative headings in degrees, or 'auto' for the KSC-facing 0/180 perpendicular branch")
    ...
    for token in (x.strip() for x in args.headings.split(",")):
        if not token:
            continue
        headings.append(None if token.lower() in {"auto", "inbound"} else float(token))
    ...
    for heading, altitude, speed, east_offset, north_offset in itertools.product(
        ...
        heading_slug = "auto" if heading is None else f"{heading:g}"
        slug = f"h{heading_slug}-a{altitude:g}-v{speed:g}"
    ...
    manifest = {
        ...
        "headings_deg": ["auto" if h is None else h for h in headings],
        "heading_default_policy": "KSC-facing runway-perpendicular: south->0deg, north->180deg",
        "altitudes_m": altitudes, "airspeeds_mps": speeds,
