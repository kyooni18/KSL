...
from telemetry_web import (
    ...
    hydrate_archive_replay_frames,
    replay_collector_loop,
    SnapshotStore,
...
class TelemetryWebLineageTests(unittest.TestCase):
    ...
    def test_simulation_replay_normalizes_exact_controller_frames_for_web_ui(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(entry["replayCandidateTrajectory"], reference)
            ...
            self.assertEqual(terminal["replayCandidateTrajectory"], [])

    def test_archive_replay_uses_dense_simulator_timeline_with_guidance_overlay(self) -> None:
        guidance = [
            {
                "phase": "ENTRY",
                "telemetry": {"ut": 100.0},
                "command": {"targetAoA": 18.0},
                "predictedTrajectory": [{"ut": 110.0, "latitude": 1.0, "longitude": 2.0}],
            },
            {
                "phase": "TAEM",
                "telemetry": {"ut": 101.0},
                "command": {"targetAoA": 12.0},
                "predictedTrajectory": [],
            },
        ]
        simulator = [
            {
                ...
                "telemetry": {"ut": 100.0, "latitude": 1.0, "longitude": 2.0, "meanAltitude": 50000.0},
                "command": {"targetAoA": 18.0},
            },
            {
                "ut": 100.5,
                "simElapsedSeconds": 0.5,
                "telemetry": {"ut": 100.5, "latitude": 1.1, "longitude": 2.1, "meanAltitude": 49000.0},
                "command": {"targetAoA": 18.0},
            },
            {
                "ut": 101.0,
                "simElapsedSeconds": 1.0,
                "telemetry": {"ut": 101.0, "latitude": 1.2, "longitude": 2.2, "meanAltitude": 48000.0},
                "command": {"targetAoA": 12.0},
            },
        ]
        frames = hydrate_archive_replay_frames(guidance, simulator, {})
        self.assertEqual(len(frames), 3)
        self.assertEqual([frame["telemetry"]["meanAltitude"] for frame in frames], [50000.0, 49000.0, 48000.0])
        self.assertEqual(frames[0]["phase"], "ENTRY")
        self.assertEqual(frames[-1]["phase"], "TAEM")
        self.assertEqual(frames[1]["command"]["targetAoA"], 18.0)
        self.assertEqual(len(frames[-1]["actualTrajectory"]), 3)
        self.assertEqual(frames[0]["connectionStatus"], "connected")

    def test_replay_collector_plays_simulator_only_archive(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            run_dir = root / "ShuttleSim" / "runs" / "sim-only"
            run_dir.mkdir(parents=True)
            packets = [
                {
                    "source": "sim", "ut": 100.0, "sim_time": 0.0, "sim_rate": 1.0,
                    "position": {"lat_deg": 1.0, "lon_deg": 2.0, "altitude_m": 50000.0},
                    "velocity": {"air_mps": 400.0, "surface_mps": 390.0, "vertical_mps": -20.0},
                    "attitude": {"aoa_deg": 18.0, "bank_deg": 0.0, "heading_deg": 90.0},
                    "ground": {"gear_down": False, "on_ground": False},
                },
                {
                    "source": "sim", "ut": 100.5, "sim_time": 0.5, "sim_rate": 1.0,
                    "position": {"lat_deg": 1.1, "lon_deg": 2.1, "altitude_m": 49000.0},
                    "velocity": {"air_mps": 380.0, "surface_mps": 370.0, "vertical_mps": -25.0},
                    "attitude": {"aoa_deg": 17.0, "bank_deg": 2.0, "heading_deg": 90.0},
                    "ground": {"gear_down": False, "on_ground": False},
                },
            ]
            telemetry_path = run_dir / "simulator-telemetry.jsonl"
            telemetry_path.write_text("".join(json.dumps(packet) + "\n" for packet in packets), encoding="utf-8")
            (run_dir / "manifest.json").write_text(json.dumps({
                "runId": "sim-only", "state": "finished", "mode": "closed-loop",
                "prerollFinal": {"ut": 100.0},
                "simulatorTelemetry": "simulator-telemetry.jsonl",
            }), encoding="utf-8")

            listed = list_simulation_runs(root)
            self.assertEqual(len(listed), 1)
            self.assertTrue(listed[0]["hasReplay"])

            self.assertGreater(listed[0]["simulatorReplayBytes"], 0)

            store = SnapshotStore()
            replay_collector_loop(
                store, root, threading.Event(), run_dir / "guidance-snapshots.jsonl",
                replay_speed=1000.0, replay_loop=False,
            )
            _version, snapshot = store.current()
            self.assertEqual(snapshot["telemetry"]["meanAltitude"], 49000.0)
            self.assertEqual(snapshot["simulation"]["state"], "replay-finished")
            self.assertEqual(snapshot["simulation"]["replayCount"], 2)
            self.assertEqual(len(snapshot["actualTrajectory"]), 2)

    def test_legacy_flat_run_artifacts_are_catalogued_and_replayable(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            runs_root = root / "ShuttleSim" / "runs"
            runs_root.mkdir(parents=True)
            guidance_path = runs_root / "legacy-flight-guidance.jsonl"
            guidance_path.write_text(json.dumps({
                "type": "snapshot",
                "snapshot": {"phase": "ENTRY", "telemetry": {"ut": 100.0}},
            }) + "\n", encoding="utf-8")
            simulator_path = runs_root / "legacy-flight-sim.jsonl"
            simulator_path.write_text(json.dumps({
                "source": "sim", "type": "telemetry", "scenario": "legacy-scenario",
                "ut": 100.0, "sim_time": 0.0, "sim_rate": 1.0,
                "position": {"lat_deg": 1.0, "lon_deg": 2.0, "altitude_m": 50000.0},
                "velocity": {"air_mps": 400.0, "surface_mps": 390.0, "vertical_mps": -20.0},
                "attitude": {"aoa_deg": 18.0, "bank_deg": 0.0, "heading_deg": 90.0},
                "ground": {"gear_down": False, "on_ground": False},
            ...
            listed = list_simulation_runs(root)
            row = next(item for item in listed if item["runId"] == "legacy-flight")
            self.assertTrue(row["hasReplay"])

            self.assertEqual(row["scenario"], "legacy-scenario")

            replay = load_simulation_replay(root, "legacy-flight", max_frames=20)
            self.assertIsNotNone(replay)
            self.assertEqual(len(replay["frames"]), 1)
            self.assertEqual(len(replay["simulatorFrames"]), 1)
            self.assertEqual(replay["run"]["mode"], "legacy-replay")
