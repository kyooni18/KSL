...
int main(void) {
    ...
    /* A fixed HAC transition owns two disjoint remaining segments: acquisition
       lead and frozen cone.  hac_remaining is deliberately poisoned here so a
       regression that adds legacy circle state again becomes immediately visible. */
    GuidanceMachine fixed = g;
    fixed.fixed_alignment_hac_latched = true;
    fixed.hac_transition_heading_cone = true;
    fixed.hac_transition_active = true;
    fixed.hac_transition_lead_length = 4000.0;
    fixed.hac_transition_lead_progress = 0.5;
    fixed.hac_transition_cone_arc_length = 12000.0;
    fixed.hac_transition_length = 12000.0;
    fixed.hac_transition_progress = 0.25;
    fixed.hac_remaining = 999999.0;
    c.guidance.final_approach_distance = 8000.0;
    RunwayCaptureEnvelope fixed_env = decision_runway_capture_envelope(&fixed, &t,
        90.0, &p, &c);
    assert(fixed_env.valid);
    double fixed_expected_path = 2000.0 + 9000.0 + 8000.0;
    assert(fabs(fixed_env.time_available_s-fixed_expected_path/t.horizontal_speed)<1e-9);
