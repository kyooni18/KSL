...
static bool sim_read_telemetry(KRPCSession*s,Telemetry*t,VehicleState*state,char*error,size_t error_size){
    ...
    buf[n]=0;if(!shuttle_sim_decode_telemetry(buf,&s->sim_planet,s->has_last_telemetry?&s->last_telemetry:NULL,t,state,error,error_size))return false;
    s->sim_need_packet=false;return true;
}
...
static bool sim_send_guidance(KRPCSession*s,const GuidanceCommand*command,bool step,char*error,size_t error_size){
    ...
    if(command){if(command->has_target_aoa&&isfinite(command->target_aoa))aoa=command->target_aoa;if(isfinite(command->target_roll))bank=command->target_roll;gear=command->gear;brakes=command->brakes;}
    char msg[512];snprintf(msg,sizeof(msg),"{\"type\":\"guidance\",\"aoa_deg\":%.9f,\"bank_deg\":%.9f,\"gear_down\":%s,\"brakes\":%s,\"step\":%s}",
        aoa,bank,gear?"true":"false",brakes?"true":"false",step?"true":"false");
...
bool krpc_apply(KRPCSession *s,const GuidanceCommand *command,unsigned airbrake_group,const char *phase,const char *status,const char *warning,const Trajectory *hud_predicted_trajectory,const Trajectory *hud_reference_trajectory,KRPCApplyResult *result,char *error,size_t error_size){
    ...
    if(s->simulator){
        ...
        if(result){memset(result,0,sizeof(*result));result->applied=true;result->autopilot_engaged=true;result->gear=command->gear;result->brakes=command->brakes;result->airbrakes=false;
            result->target_pitch=command->target_pitch;result->target_heading=command->target_heading;result->target_roll=command->target_roll;result->throttle=0;
