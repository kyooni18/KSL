...
static bool hac_transition_plan(HACTransitionPlan*out,double current_e,double current_n,
    ...
    for(int ai=0;ai<arc_samples;ai++){
        ...
        double transition_budget=fmax(chord,max_fixed_path-response_distance-arc*hac_radius);
...
static void hac_transition_store(GuidanceMachine*g,const HACTransitionPlan*p,double response_time){
    ...
    if(!g->hac_transition_active){g->hac_transition_length=0;g->hac_transition_lead_length=0;g->hac_transition_exit_speed=0;return;}
...
static GuidanceResult stabilized(GuidanceMachine*g,GuidanceResult r,const Telemetry*t,const VehicleProfile*v,const GuidanceSettings*s,double dt){
    ...
    /*
     * Guidance owns geometry and safety envelopes; FlightControl owns actuator
     * bandwidth. Do not insert a second phase-tuned rate controller here.
     * The native FCS observes pitch/roll/yaw acceleration from torque/inertia
     * and live response, then applies a stopping-distance controller before
     * surfaces are written.
     *
     * This layer only sanitizes invariant references and records the requested
     * high-level reference for diagnostics/recovery.
     */
    ...
    if(g->pitch_limiter.has_value&&dt>0.0&&isfinite(dt))
        g->pitch_limiter.rate=(pitch_reference-g->pitch_limiter.value)/dt;
    else g->pitch_limiter.rate=0.0;
    g->pitch_limiter.value=pitch_reference;
    g->pitch_limiter.has_value=true;

    double roll_reference=norm_signed_deg(r.command.target_roll);
    if(g->roll_limiter.has_value&&dt>0.0&&isfinite(dt)){
        /* This is a measured-reference diagnostic, not a command slew. A
           committed S-turn reversal can legitimately move the desired bank
           by roughly 70 degrees in one sample; exposing that raw step as a
           reference rate makes control_recovery_needed classify the planned
           reversal as an oscillation. Bound only the diagnostic to the same
           phase envelope used by recovery; FlightControl still receives the
           full geometry target below. */
        double reference_delta=norm_signed_deg(roll_reference-
            norm_signed_deg(g->roll_limiter.value));
        double reference_rate_limit=s->entry_roll_rate;
        if(r.phase==PHASE_TAEM||r.phase==PHASE_HEADING_ALIGNMENT)
            reference_rate_limit=s->taem_roll_rate;
        else if(r.phase==PHASE_FINAL||r.phase==PHASE_FLARE)
            reference_rate_limit=s->approach_roll_rate;
        reference_rate_limit=fmax(0.0,reference_rate_limit);
        g->roll_limiter.rate=reference_rate_limit>0.0?
            clampd(reference_delta/dt,-reference_rate_limit,
                reference_rate_limit):0.0;
    }else g->roll_limiter.rate=0.0;
    g->roll_limiter.value=roll_reference;
...
static double entry_program_final_s_turn_drag_aoa(const GuidanceMachine*g,
    ...
    double height=fabs(t->mean_altitude-q->altitude);
    double air_path=hypot(path_distance,height);
    if(!(air_path>DBL_MIN)||!isfinite(air_path))return NAN;

    double measured_drag=live_drag_accel(t,aero,v);
    double current_work=fmax(0.0,measured_drag)*air_path;
    EnergyPathEnvelope current_energy=decision_energy_path_envelope(
        t,p,available,current_work,air_path);
    ...
    const int samples=2*DBL_MANT_DIG;
    double best=base,best_error=INFINITY;
    for(int i=0;i<=samples;i++){
        ...
        double predicted_drag=(measured_drag>0.0&&current_df>DBL_MIN)?
            measured_drag*df/current_df:measured_drag;
        double work=fmax(0.0,predicted_drag)*air_path;
        double error=fabs(work-target_work);
        if(error<best_error){
            best_error=error;
            best=aoa;
        }
    ...
    double resolution=sqrt(DBL_EPSILON)*fmax(1.0,ceiling);
    return best>base+resolution?best:NAN;
}
...
static bool entry_program_rollthrough_endpoint_ready(const GuidanceMachine*g,const Telemetry*t,
    ...
    EntryLateralTurnInput turn_input={
        ...
        .capture_radius_m=cfg->guidance.mm304_handoff_radius,
...
static void entry_taem_turnability_target(const GuidanceMachine*g,const Telemetry*t,
    ...
    /* The owned side is also the signed bank/course-rate side in the runway
       frame: positive bank produces positive cross-track course rate.  The
       previous subtraction published the opposite perpendicular inlet after
       the first-side latch, so MM304 could hold a negative bank while chasing
       the +90-degree outlet (and vice versa). */
...
bool guidance_begin_hac_test(GuidanceMachine*g,const Telemetry*t,double course,const PlanetModel*p,AerodynamicModel aero,const LandingConfiguration*cfg,char*message,size_t message_size){
    ...
    g->hac_side=t->runway_cross_track>=0?-1:1;g->hac_side_selected=false;g->hac_radius=s->hac_radius;
...
static double terminal_integrated_energy_path(const GuidanceMachine*g,const Telemetry*t,const PlanetModel*p,
    ...
    for(int i=0;i<DBL_MANT_DIG;i++){
        ...
        if(hi-lo<=sqrt(DBL_EPSILON)*scale)break;
        double mid=.5*(lo+hi);
...
static bool terminal_test_start_spiral(GuidanceMachine*g,const Telemetry*t,double course,const PlanetModel*p,
    ...
    for(int ri=0;ri<radius_samples;ri++){
        ...
        for(int side=-1;side<=1;side+=2){
            ...
            if(violation<=sqrt(DBL_EPSILON)){
                bool better=path_error<best_path_error||
    ...
    if(getenv("KSP_LANDER_HAC_DIAGNOSTICS")&&
       strcmp(getenv("KSP_LANDER_HAC_DIAGNOSTICS"),"1")==0){
        fprintf(stderr,
            "HAC constrained selector: UT %.1f degraded=%d R %.0f side %+.0f pathErr %.4f violation %.4f controlUse %.4f slope %.2f energySlope %.2f\n",
...
static bool terminal_candidate_refresh_allowed(const GuidanceMachine*g,
    ...
    return expired&&terminal_candidate_constraint_order(a,b)<=0;
}
...
static bool terminal_candidate_from_future(const GuidanceMachine*g,const Telemetry*live,
    ...
    if(getenv("KSP_LANDER_HAC_DIAGNOSTICS")&&strcmp(getenv("KSP_LANDER_HAC_DIAGNOSTICS"),"1")==0){
        fprintf(stderr,"terminal topology: UT %.1f future alt %.0f V %.0f range %.0f spline=%s geom=%d energy=%d degraded=%d q %.2f len %.0f peakLat %.3f rate %.3f slope %.2f violation %.3f chosen=%s\n",
            ...
            spline.slope,spline.join.violation_score,
    ...
    if(terminal_test_start_spiral(&trial,future,future_course,p,aero,cfg)){
        ...
        if(getenv("KSP_LANDER_HAC_DIAGNOSTICS")&&strcmp(getenv("KSP_LANDER_HAC_DIAGNOSTICS"),"1")==0){
            fprintf(stderr,"terminal topology HAC: UT %.1f geom=%d energy=%d degraded=%d q %.2f chosen=%s\n",
    ...
    if(!g->terminal_planning_deferred&&
       t->ut-g->terminal_prediction_ut>=prediction_refresh){
        ...
        if(g->terminal_region_entered){
            ...
            if(live_energy_ready&&!best_energy_ready){
                best=live_origin;
                best_future=*t;
                best_horizon=0.0;
            }
...
static GuidanceResult taem_guidance(GuidanceMachine*g,const Telemetry*t,double course,const PlanetModel*p,AerodynamicModel aero,const LandingConfiguration*cfg,const Trajectory*ref,double dt){
    const VehicleProfile*v=&cfg->vehicle;const GuidanceSettings*s=&cfg->guidance;GuidanceSettings path_settings=terminal_path_settings(g,s);const GuidanceSettings*hs=&path_settings;double radius=g->terminal_glide_mode?fmax(s->hac_radius,g->hac_radius):fmax(s->hac_radius,g->hac_radius);
    ...
    HACPhaseGuidance phase_path=hac_phase_guidance(lateral_stage,g,t,&cfg->site,hs,p->radius,
        g->hac_side,planet_surface_gravity(p),course,radius);
    if(!phase_path.valid){
        GuidanceCommand safe=atmospheric(t,t->ground_track_heading,0.0,v,0.0,false,PROFILE_TAEM);
        safe.heading_control_enabled=false;safe.has_target_aoa=true;
    ...
    double path_remaining=g->hac_transition_active?h.arc_remaining:g->hac_remaining;
    double hac_slope=g->hac_circuit_slope>0?g->hac_circuit_slope:(g->terminal_glide_mode&&g->terminal_test_glide_slope>0?g->terminal_test_glide_slope:hs->taem_glide_slope);
    if(g->terminal_glide_mode){
        double final_entry_altitude=cfg->site.altitude+hs->final_approach_distance*tan(g->terminal_test_glide_slope*DEG2RAD);
...
static void terminal_invalidate_frozen_path(GuidanceMachine*g){
    ...
    g->hac_transition_active=false;g->hac_transition_progress=0;g->hac_transition_lead_progress=0;
...
static GuidanceResult terminal_guidance(GuidanceMachine *g, const Telemetry *t, const VehicleState *state, double course,
    ...
    if(g->terminal_glide_mode&&g->terminal_test_capture_active){
        ...
        if(terminal_candidate_commit_ready(g,t,p,aero,cfg)){
            ...
            if(!margin_exhausted&&g->hac_capture_lost_duration>=planning_grace&&
