...
EntryPrediction predictor_simulate_entry_guidance_shadow_with_terminal_model(VehicleState state,const Telemetry *telemetry,const GuidanceMachine *guidance,const DeorbitPlan *plan,const PlanetModel *planet,AerodynamicModel aero,const AerodynamicEnvelope *env,const TrajectoryCalibrationModel *cal,const LandingConfiguration *cfg,const TerminalModel *terminal_model,double max_duration,bool include_trajectory);
...
double entry_taem_tangent_target_hac_side(const TaemInterfaceTarget *target,
