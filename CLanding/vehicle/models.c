...
void landing_configuration_normalize(LandingConfiguration *c){
    ...
    if(c->connection.rpc_host[0] &&
       (c->connection.rpc_port<1 || c->connection.rpc_port>65535))
        c->connection.rpc_port=d.connection.rpc_port;
...
void landing_configuration_json(JsonWriter *w,const LandingConfiguration *c){
    jw_char(w,'{');KEY(w,"connection");jw_char(w,'{');KEY(w,"serialPort");jw_string(w,c->connection.serial_port);COMMA(w);KEY(w,"rpcHost");jw_string(w,c->connection.rpc_host);COMMA(w);KEY(w,"rpcPort");jw_integer(w,c->connection.rpc_port);COMMA(w);KEY(w,"baudRate");jw_integer(w,c->connection.baud_rate);COMMA(w);KEY(w,"timeoutMs");jw_integer(w,c->connection.timeout_ms);COMMA(w);KEY(w,"clientName");jw_string(w,c->connection.client_name);jw_char(w,'}');COMMA(w);
...
#define TN(K,F) KEY(w,K);jw_number(w,t->F);COMMA(w)
    ...
    KEY(w,"gear");jw_bool(w,t->gear);COMMA(w);KEY(w,"brakes");jw_bool(w,t->brakes);COMMA(w);KEY(w,"airbrakes");if(t->has_airbrakes)jw_bool(w,t->airbrakes);else jw_null(w);COMMA(w);KEY(w,"hasMainGearGrounded");jw_bool(w,t->has_main_gear_grounded);COMMA(w);KEY(w,"mainGearGrounded");jw_bool(w,t->main_gear_grounded);COMMA(w);KEY(w,"hasNoseGearGrounded");jw_bool(w,t->has_nose_gear_grounded);COMMA(w);KEY(w,"noseGearGrounded");jw_bool(w,t->nose_gear_grounded);COMMA(w);KEY(w,"vesselSituation");jw_string(w,t->vessel_situation);COMMA(w);
...
static void entry_feedback_json(JsonWriter*w,const EntryGuidanceFeedback*f){
    jw_char(w,'{');KEY(w,"valid");jw_bool(w,f->valid);COMMA(w);
    KEY(w,"lateralValid");jw_bool(w,f->lateral_valid);COMMA(w);
    KEY(w,"ut");nullable_number(w,f->valid&&isfinite(f->ut),f->ut);COMMA(w);
    KEY(w,"confidence");nullable_number(w,f->valid&&isfinite(f->confidence),f->confidence);COMMA(w);
    KEY(w,"measuredDrag");nullable_number(w,f->valid&&isfinite(f->measured_drag_accel),f->measured_drag_accel);COMMA(w);
    KEY(w,"referenceDrag");nullable_number(w,f->valid&&isfinite(f->reference_drag_accel),f->reference_drag_accel);COMMA(w);
    KEY(w,"dragError");nullable_number(w,f->valid&&isfinite(f->drag_error_accel),f->drag_error_accel);COMMA(w);
    KEY(w,"predictedAchievableRange");nullable_number(w,f->valid&&isfinite(f->predicted_achievable_range),f->predicted_achievable_range);COMMA(w);
    KEY(w,"requiredRangeToGo");nullable_number(w,f->valid&&isfinite(f->required_range_to_go),f->required_range_to_go);COMMA(w);
    KEY(w,"energyMargin");nullable_number(w,f->valid&&isfinite(f->energy_margin),f->energy_margin);COMMA(w);
    KEY(w,"rangeErrorScale");nullable_number(w,f->valid&&isfinite(f->range_error_scale),f->range_error_scale);COMMA(w);
    KEY(w,"requiredVerticalLift");nullable_number(w,f->valid&&isfinite(f->required_vertical_lift_accel),f->required_vertical_lift_accel);COMMA(w);
    KEY(w,"crossrangeError");nullable_number(w,f->lateral_valid&&isfinite(f->crossrange_error),f->crossrange_error);COMMA(w);
    KEY(w,"crossrangeRate");nullable_number(w,f->lateral_valid&&isfinite(f->crossrange_rate),f->crossrange_rate);COMMA(w);
    KEY(w,"projectedCrossrangeError");nullable_number(w,f->lateral_valid&&isfinite(f->projected_crossrange_error),f->projected_crossrange_error);COMMA(w);
    KEY(w,"crossrangeCorridor");nullable_number(w,f->lateral_valid&&isfinite(f->crossrange_corridor),f->crossrange_corridor);COMMA(w);
    KEY(w,"energyBankMagnitude");nullable_number(w,f->lateral_valid&&isfinite(f->energy_bank_magnitude_deg),f->energy_bank_magnitude_deg);COMMA(w);
    KEY(w,"commandedBank");nullable_number(w,f->valid&&isfinite(f->commanded_bank_deg),f->commanded_bank_deg);COMMA(w);
    KEY(w,"bankSign");nullable_number(w,f->valid&&isfinite(f->bank_sign),f->bank_sign);COMMA(w);
    KEY(w,"reversalRequested");jw_bool(w,f->reversal_requested);jw_char(w,'}');
}
